...
function ModifyAccountModal({ data, isShowModal, setIsShowModal }: Props) {
  ...
  const { places, categories } = useAppSelector((state) => state.adminGeneral);
    ...
    if (isShowModal === "update") {
      ...
      categoryApi
        ...
        .then((res: any) => {
          ...
          result.splice(index, 1, res.category);
          dispatch(
            setGeneral({
              categories: result,
            })
...
function Categories() {
  const { loading, categories } = useAppSelector((state) => state.adminGeneral);
  ...
  const tableData = categories.map((item: any, index: any) => {
    return {
      key: index,
      ...item,
    };
  ...
  return (
    <Style.AdminPlaceWrap>
      <Style.CustomSpaceBox>
        <Style.Title>Quản lý loại hình</Style.Title>
        <Style.CustomSpace>
          <Style.CustomButton
            type="primary"
            onClick={() => {
