...
const UpdateForm = ({ review, onClose }: { review: any; onClose: any }) => {
  ...
  useEffect(() => {
    if (review) {
      setRate({
        rateDrink: review.rateDrink,
        ratePosition: review.ratePosition,
        ratePrice: review.ratePrice,
        rateService: review.rateService,
        rateView: review.rateView,
      });
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (images.length === 0 && !content) {
      return message.error("Vui lòng điền đầy đủ thông tin và ảnh!");
    }
  ...
  return (
    <React.Fragment>
      <Styled.ModalHeader>
        <div className="title">
          <h2>Đánh giá {review?.place?.name || "địa điểm"}</h2>
        </div>
